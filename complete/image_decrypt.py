...
print "decrypting image: ", image_path
...
while index < len(message):
    if index + 16*128 > len(message):
         decipered_text = decipered_text + EME_D(T,K,message[index:len(message)])
         break
    decipered_text = decipered_text + EME_D(T,K,message[index:index+16*128])
    index = index + 16*128
